...
const geral = require('./model/gerais.js');
const psql = require("./model/connector.js");
const services = require('./services/emais.js');
...
app.listen(PORT, async () => {
    console.log("Iniciando Express na PORT " + PORT);
...
const BACK = path.join(pastaView, "BACK-END");
const GRW = path.join(BACK, "GrowSite");
const FRONT = path.join(pastaView, "FRONT-END");
const BANCO = path.join(pastaView, "BANCO DE DADOS");
...
app.use(express.static(BANCO));
app.use(express.static(GRW));
...
app.post('/existeEmail', async (req, res) => {
    const dados = req.body; // Certifique-se de que o body-parser está configurado corretamente
    try {
